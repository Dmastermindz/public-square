...
const dashboardWidget = ({ linkedWalletAddress, embeddedWalletAddress, walletBalance, ready, selected }) => {
  ...
  //Use Effect to Load ETH Balance Through Privy Wallet
  useEffect(() => {
    ...
    if (linkedWalletAddress) {
      ...
      const url = `https://api.basescan.org/api?module=account&action=balance&address=${linkedWalletAddress}&apikey=${apiKey}`;
      console.log("URL:", url);
      ...
      fetch(url)
        ...
        .then((data) => {
          ...
          console.log(`The User's ETH balance is: ${bnbBalance}`);
        })
        .catch((error) => console.error("Error fetching User ETH Account Balance data:", error));
    }
    ...
    if (embeddedWalletAddress && !linkedWalletAddress) {
      ...
      const url = `https://api.basescan.org/api?module=account&action=balance&address=${embeddedWalletAddress}&apikey=${apiKey}`;
      console.log("URL:", url);
  ...
  return (
    <div className="flex shadowz items-center px-8 space-x-0.5 py-[40px] flex-row h-1/3 mt-[50px] w-full bg-[#1d1f31] bg-opacity-80 rounded-lg transition-transform hover:scale-[101%] duration-300">
      <div className="flex-1 text-center">
        {/* <h1 className="text-lg font-semibold tracking-wider ">ETH Balance</h1>
        <h1 className="text-sm">{`${Number(walletBalance).toPrecision(2).toLocaleString("en-US", { maximumFractionDigits: 0 })} ETH`}</h1>
        ...
        {/* <h1 className="text-sm">{`${Number(walletBalance).toPrecision(2).toLocaleString("en-US", { maximumFractionDigits: 0 })} USD`}</h1> */}
        <h1 className="text-sm">${(walletBalance * bnbPrice).toLocaleString("en-US", { maximumFractionDigits: 2 })}</h1>
      </div>
